...
#endif
{
  ...
  //  Cofigure and Force ATM Hardware Reset
  pinMode(RST_ATM, OUTPUT);
  digitalWrite(RST_ATM, LOW);
  delayMicroseconds(50);
  digitalWrite(RST_ATM, HIGH);
...
// Quick Peek at Current Register Values
unsigned short ATM90E3x::ListRegisters()
{

  Serial.println("=====================================");

  Serial.println("ATM Registers");
  ...
  for (uint8_t reg = 0x00; reg <= 0x7F; reg++)
  {
    ...
    uint16_t value = (Read32Register(reg, 0xFFFF) >> 16) & 0xFFFF;
    Serial.print("0x");
    if (value < 0x1000)
      Serial.print("0");
    if (value < 0x0100)
