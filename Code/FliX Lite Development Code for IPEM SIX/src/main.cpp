...
void setup()
{
  ...
  Serial.println("FLiX Lite - Integrated Firmware for DitroniX Power Energy Monitor SDK Boards\n");
...
// the 2nd (B) current channel is not used with the split phase meter
  ...
  // eic.ListRegisters(); // All Registers
...
void loop() // This loop tests the various GPIO ports and also outputs read data.
{
  ...
  // Cycle OPTO_OUT Output (Seen on LED)
  digitalWrite(OPTO_OUT, HIGH);
  delay(50);
  digitalWrite(OPTO_OUT, LOW);

  // Cycle GPO_PWM Output (Seen on LED)
  digitalWrite(GPO_PWM, HIGH);
  delay(50);
  digitalWrite(GPO_PWM, LOW);

  // Cycle USER_LED Output (Seen on LED) INVERTED
